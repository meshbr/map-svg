...
function mapsvg_add_jscss_common(){
    ...
    if(MAPSVG_DEBUG){
        ...
        wp_register_script('handlebars', MAPSVG_PLUGIN_URL . 'js/handlebars.js', null, '4.7.7'.MAPSVG_ASSET_VERSION);
...
function mapsvg_add_jscss_admin($hook_suffix){
    ...
    if(isset($_GET['page']) && $_GET['page']=='mapsvg-config'){
	    ...
	    if(MAPSVG_DEBUG){
	        ...
	        wp_register_script('bootstrap', MAPSVG_PLUGIN_URL . "js/bootstrap.min.js", null, '3.3.7');
	        wp_enqueue_script('bootstrap');
	        wp_register_style('bootstrap', MAPSVG_PLUGIN_URL . "css/bootstrap.min.css", null, '3.3.7');
...
function mapsvg_print( $atts ){
    ...
    add_action('wp_footer', 'mapsvg_script', 9998);
...
function mapsvg_create_new_map($svg_file_url_path){
	...
	$obj2 = new stdClass();
	$obj2->{'1'} = array("label"=>"First","value"=>'1');
	$obj2->{'2'} = array("label"=>"Second","value"=>'2');
	$obj2->{'3'} = array("label"=>"Third","value"=>'3');
	...
	$db_schema = array(
        ...
        array(
            ...
            'name'=>'category',
            'label'=>'Category',
            'options'=> array($obj2->{'1'},$obj2->{'2'},$obj2->{'3'}),
            'optionsDict'=> $obj2,
            'visible'=>'true'
        ),
        array(
...
function ajax_mapsvg_save_svg(){
	...
	$root = ABSPATH;
	$root = wp_normalize_path($root);
	$svg_file_path = stripslashes($_POST['path']);
	...
	*/
	...
	if (!move_uploaded_file($_FILES["file"]["tmp_name"], $target_file)) {
		echo '{"status": "Error", "error": "Can\'t save the file"}';
		die();
	}

    $auto_update = get_post_meta($_POST['map_id'], 'mapsvg_manual_regions', true);

    mapsvg_set_regions_table($_POST['map_id'], $_POST['path'], '');
...
function mapsvg_encode_data($map_id, $table, $data, $convert_latlng_to_address = false){
    ...
    global $db_schema, $db_types, $db_options, $db_multi, $wpdb, $db_post_type;
            ...
            case "post":
                if($key === 'post'){
                    if(!is_numeric($value) && strlen($value)>0){
	                    $post = get_page_by_path($value, OBJECT, $db_post_type);
...
function mapsvg_add_jscss_common_2(){
    ...
    wp_register_script('handlebars', MAPSVG_PLUGIN_URL . 'mapsvg2/js/handlebars.js', null, '4.6.2');
    wp_enqueue_script('handlebars');
...
function mapsvg_add_jscss_admin_2(){
    ...
    if(isset($_GET['page']) && $_GET['page']=='mapsvg-config'){
        ...
        wp_register_script('bootstrap', MAPSVG_PLUGIN_URL . "mapsvg2/js/bootstrap.min.js", null, '3.3.7');
        wp_enqueue_script('bootstrap');
        wp_register_style('bootstrap', MAPSVG_PLUGIN_URL . "mapsvg2/css/bootstrap.min.css", null, '3.3.7');
        wp_enqueue_style('bootstrap');
...
function mapsvg_print_2( $atts ){
    ...
    //wp_footer('script');
    add_action('wp_footer', 'mapsvg_script', 9999);
