(function (global, factory) {
    ...
    class Marker extends MapObject {
        ...
        getBubbleSize() {
            ...
            if (this.object[this.mapsvg.options.choropleth.sourceField]) {
                let maxBubbleSize = Number(this.mapsvg.options.choropleth.bubbleSize.max), minBubbleSize = Number(this.mapsvg.options.choropleth.bubbleSize.min), maxSourceFieldvalue = this.mapsvg.options.choropleth.coloring.gradient.values.max, minSourceFieldvalue = this.mapsvg.options.choropleth.coloring.gradient.values.min, sourceFieldvalue = parseFloat(this.object[this.mapsvg.options.choropleth.sourceField]);
                bubbleSize = ((sourceFieldvalue - minSourceFieldvalue) / (maxSourceFieldvalue - minSourceFieldvalue)) * (maxBubbleSize - minBubbleSize) + Number(minBubbleSize);
            }
            else {
                bubbleSize = false;
            }
        ...
        drawBubble() {
            ...
            if (bubbleValue) {
                if ($$5('#' + bubbleId).length === 0) {
                    this.bubble = $$5('<div id="' + bubbleId + '" data-marker-id="' + this.element.id + '" class="mapsvg-bubble mapsvg-marker-bubble"></div>')[0];
                    $$5(this.mapsvg.layers.markers).append(this.bubble);
                }
                let color = this.getChoroplethColor(), bubbleSize = Number(this.getBubbleSize()), pos = this.getBubbleScreenPosition();
                $$5(this.bubble).css('transform', 'translate(' + pos.x + 'px,' + pos.y + 'px)')
                    .css('background-color', color)
                    .css('width', bubbleSize + 'px')
                    .css('height', bubbleSize + 'px')
                    .css('lineHeight', (bubbleSize - 2) + 'px');
                this.setBubblePropagateEvents(true);
            }
            else {
                $$5('#' + bubbleId).remove();
            }
        ...
        setBubbleMode() {
            this.bubbleMode = this.mapsvg.options.choropleth.on && this.mapsvg.options.choropleth.source == 'database' && this.mapsvg.options.choropleth.bubbleMode;
            if (this.bubbleMode) {
                this.drawBubble();
            }
        }
        setBubblePropagateEvents(state) {
        }
    }
    ...
    class Region extends MapObject {
        ...
        getChoroplethColor() {
            ...
            if (!regionValue) {
                ...
                let gradient = this.mapsvg.options.choropleth.coloring.gradient, w = gradient.values.maxAdjusted === 0 ? 0 : (regionValue - gradient.values.min) / gradient.values.maxAdjusted, r = Math.round(gradient.colors.diffRGB.r * w + gradient.colors.lowRGB.r), g = Math.round(gradient.colors.diffRGB.g * w + gradient.colors.lowRGB.g), b = Math.round(gradient.colors.diffRGB.b * w + gradient.colors.lowRGB.b), a = (gradient.colors.diffRGB.a * w + gradient.colors.lowRGB.a).toFixed(2);
                color = 'rgba(' + r + ',' + g + ',' + b + ',' + a + ')';
            }
            else {
                let paletteColors = this.mapsvg.options.choropleth.coloring.palette.colors;
                ...
                else if (!paletteColors[paletteColors.length - 1].valueTo && regionValue > paletteColors[paletteColors.length - 1].valueFrom) {
                    color = paletteColors[paletteColors.length - 1].color;
                }
                else {
                    paletteColors.forEach(function (paletteColor) {
                        if (regionValue >= paletteColor.valueFrom && regionValue < paletteColor.valueTo) {
                            color = paletteColor.color;
                        }
                    });
                    color = color ? color : this.mapsvg.options.choropleth.coloring.palette.outOfRange.color;
        ...
        getBubbleSize() {
            ...
            if (this.data[this.mapsvg.options.choropleth.sourceField]) {
                ...
                bubbleSize = ((sourceFieldvalue - minSourceFieldvalue) / (maxSourceFieldvalue - minSourceFieldvalue)) * (maxBubbleSize - minBubbleSize) + minBubbleSize;
            }
            else {
                bubbleSize = false;
            }
        ...
        drawBubble() {
            if (this.data) {
                ...
                if (bubbleValue) {
                    ...
                    let color = this.getChoroplethColor();
                    let bubbleSize = Number(this.getBubbleSize());
    ...
    class MapSVGMap {
        ...
        setChoropleth(options) {
            ...
            options.on != undefined && (options.on = MapSVG.parseBoolean(options.on));
            if (typeof options.coloring === 'undefined' || typeof options.coloring.palette === 'undefined' || typeof options.coloring.palette.colors === 'undefined') {
                ...
                let paletteColorsIndexes = Object.keys(options.coloring.palette.colors);
                if ((paletteColorsIndexes.length > 1) || (paletteColorsIndexes[0] == '0' && Object.keys(options.coloring.palette.colors[0]).length > 1)) {
                    _this.options.choropleth.coloring.palette.colors = options.coloring.palette.colors;
                }
                else {
                    let paletteColorIndex = Object.keys(options.coloring.palette.colors)[0];
                    $$x.extend(true, _this.options.choropleth.coloring.palette.colors[paletteColorIndex], options.coloring.palette.colors[paletteColorIndex]);
                }
            }
            _this.updateChoroplethMinMax();
            ...
            $$x(_this.containers.map).find('.mapsvg-choropleth-legend').remove();
            if (_this.options.choropleth.on && _this.options.choropleth.coloring.legend.on) {
                let legend = _this.options.choropleth.coloring.legend, coloring = '';
                if (_this.options.choropleth.coloring.mode === 'gradient') {
                    for (let chunkIdx = 1; chunkIdx < 5; chunkIdx++) {
                        coloring += '<div class="mapsvg-choropleth-legend-gradient-chunk">' + ((_this.options.choropleth.coloring.gradient.values.maxAdjusted * chunkIdx / 5) + _this.options.choropleth.coloring.gradient.values.min) + '</div>';
                    }
                }
                else {
                    let paletteColors = _this.options.choropleth.coloring.palette.colors;
                    paletteColors.forEach(function (paletteColor, idx) {
                        ...
                        coloring +=
                            '<div class="mapsvg-choropleth-legend-palette-color-wrap" data-idx="' + idx + '">' +
                                '<div class="mapsvg-choropleth-legend-palette-color"></div>' +
                                '<div class="mapsvg-choropleth-legend-palette-color-description">' + paletteColorElementText + '</div>' +
                                '</div>';
                    });
                }
                _this.containers.legend = {
                    title: $$x('<div class="mapsvg-choropleth-legend-title">' + legend.title + '</div>')[0],
                    text: $$x('<div class="mapsvg-choropleth-legend-text">' + legend.text + '</div>')[0],
                    coloring: $$x('<div class="mapsvg-choropleth-legend-' + _this.options.choropleth.coloring.mode + '">' + coloring + '</div>')[0],
                    description: $$x('<div class="mapsvg-choropleth-legend-description">' + legend.description + '</div>')[0],
                    container: $$x('<div />').addClass('mapsvg-choropleth-legend mapsvg-choropleth-legend-' + legend.layout + ' mapsvg-choropleth-legend-container-' + legend.container)[0]
                };
                $$x(_this.containers.legend.container)
                    .append(_this.containers.legend.title)
                    .append(_this.containers.legend.text)
                    .append(_this.containers.legend.coloring)
                    .append(_this.containers.legend.description);
                $$x(_this.containers.map).append(_this.containers.legend.container);
                _this.setChoroplethLegendCSS();
            ...
            if (_this.options.choropleth.on && _this.options.choropleth.coloring.mode === 'gradient') {
                let colors = _this.options.choropleth.coloring.gradient.colors;
                if (colors) {
                    ...
                    _this.containers.legend && _this.setChoroplethLegendCSS();
                }
            }
            _this.redrawChoropleth();
        }
        ...
        updateChoroplethMinMax() {
            ...
            if (_this.options.choropleth.on) {
                let gradient = _this.options.choropleth.coloring.gradient, values = [];
        ...
        setChoroplethLegendCSS() {
            let gradient = this.options.choropleth.coloring.gradient, legend = this.options.choropleth.coloring.legend;
            $$x('.mapsvg-choropleth-legend').css({
                'width': legend.width,
                'height': legend.height
            });
            if (this.options.choropleth.coloring.mode === 'gradient') {
                if (legend.layout === 'horizontal') {
                    $$x('.mapsvg-choropleth-legend-horizontal .mapsvg-choropleth-legend-gradient').css({
                        'background': 'linear-gradient(to right,' + gradient.colors.low + ' 1%,' + gradient.colors.high + ' 100%)'
                    });
                }
                else {
                    $$x('.mapsvg-choropleth-legend-vertical .mapsvg-choropleth-legend-gradient').css({
                        'background': 'linear-gradient(to bottom,' + gradient.colors.low + ' 1%,' + gradient.colors.high + ' 100%)'
                    });
                }
            }
            else {
                let paletteColors = this.options.choropleth.coloring.palette.colors;
                paletteColors.forEach(function (paletteColor, idx) {
                    $$x('.mapsvg-choropleth-legend-palette-color-wrap[data-idx="' + idx + '"] .mapsvg-choropleth-legend-palette-color').css({
        ...
        redrawBubbles() {
            ...
            this.objectsRepository.objects.forEach(function (object) {
                object.location.marker.setBubbleMode();
            });
        }
        destroy() {
            var _this = this;
        ...
        pickChoroplethColor(choroplethValue) {
            var _this = this;
            var w = (choroplethValue - _this.options.choropleth.coloring.gradient.values.min) / _this.options.choropleth.coloring.gradient.values.maxAdjusted;
            var rgba = {
                r: Math.round(_this.options.choropleth.coloring.gradient.colors.diffRGB.r * w + _this.options.choropleth.coloring.gradient.colors.lowRGB.r),
                g: Math.round(_this.options.choropleth.coloring.gradient.colors.diffRGB.g * w + _this.options.choropleth.coloring.gradient.colors.lowRGB.g),
                b: Math.round(_this.options.choropleth.coloring.gradient.colors.diffRGB.b * w + _this.options.choropleth.coloring.gradient.colors.lowRGB.b),
                a: Math.round(_this.options.choropleth.coloring.gradient.colors.diffRGB.a * w + _this.options.choropleth.coloring.gradient.colors.lowRGB.a)
            };
            return rgba;
        }
